...
export default function EmailDetailPage({ params }: Props) {
  ...
  const fetchEmail = async () => {
    try {
      setIsLoading(true);
      const data = await fetchEmailByIdAction(id, user?.token);
      setEmailData(data);
    } catch (error) {
  ...
  const markAsStarredMutation = async (id: number, starred: boolean) => {
    try {
      await markEmailAsStarredAction(id, starred, user?.token);
      showCustomToast({
        title: "Email marked as " + (starred ? "starred" : "unstarred"),
        description: `Email ${id} has been marked as ${
      ...
      showCustomToast({
        title: "Error",
        description: `Failed to mark email ${id} as ${
          starred ? "starred" : "unstarred"
        }`,
        icon: starred ? (
          <span className="text-yellow-500">⭐</span>
      ...
      <div className="flex items-center justify-between p-2 bg-gray-100 rounded-xl">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            size="icon"
            onClick={() =>
              markAsStarredMutation(emailData.id, !emailData.starred)
            }
          >
            <Star
              className={`w-5 h-5 ${
          ...
          <Button
            variant="ghost"
            size="icon"
            onClick={() => deleteEmailMutation(emailData.id)}
          >
