...
resource "terracurl_request" "add_instance" {
  ...
  headers = {
    ...
    request_body = format("action=add_instance&template=%s&rid=%d&qty=1&keys[]=%s&srvrname=%s&billingcycle=%s&location_id=%d&API=%s", var.template, var.rid, urlencode(var.ssh_key_name), urlencode(var.server_name), var.billingcycle, var.location_id, urlencode(var.API))
