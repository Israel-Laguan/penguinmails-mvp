  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3 min-w-[140px]">
        <p className="font-medium text-gray-900 text-sm mb-2">{formattedDate}</p>
        <div className="space-y-1">
          <div className="flex justify-between items-center">
            <span className="text-gray-600 text-xs">Sent:</span>
            <span className="font-medium text-gray-700 text-xs">{data.sent}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-blue-600 text-xs">Opened:</span>
            <span className="font-medium text-blue-700 text-xs">{data.opened}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-green-600 text-xs">Replied:</span>
            <span className="font-medium text-green-700 text-xs">{data.replied}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-purple-600 text-xs">Clicked:</span>
            <span className="font-medium text-purple-700 text-xs">{data.clicked}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-red-600 text-xs">Bounced:</span>
            <span className="font-medium text-red-700 text-xs">{data.bounced}</span>
          </div>
        </div>
      </div>
    );
                ...
                {metrics.map((metric) => (
                  ...
                  >
                    ...
                    <div 
                      ...
                      style={{ backgroundColor: metric.visible ? metric.color : '#D1D5DB' }}
                    />
                    {metric.label}
                  </Button>
                ))}
                ...
                {metrics.map((metric) => (
                  metric.visible && (
                    <Line
                      ...
                      activeDot={{ r: 5, stroke: metric.color, strokeWidth: 2, fill: '#fff' }}
                    />
                  )
                ))}
              </LineChart>
